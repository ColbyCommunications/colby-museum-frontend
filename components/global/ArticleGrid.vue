                ...
                <div class="pagination__main">
                    <NuxtLink
                        ...
                        :to="`/${items_type == 'posts' ? 'news' : items_type}/page-${
                            ...
                            this.$route.query.maker ||
                            this.$route.query.year ||
                            this.$route.query.type ||
                            this.$route.query.sort ||
                            this.$route.query.sortby ||
                            this.$route.query.has_image ||
                            this.$route.query.chronology ||
                            this.$route.query.location ||
                            this.$route.query.variant ||
                            this.$route.query.category ||
                            this.$route.query.embark_id
    ...
    export default {
        ...
        computed: {
            filteredItems() {
                ...
                if (this.activeFilters.length > 0 && this.newItems[0]?.post) {
                    filteredItems = component.newItems.filter((item) =>
                        Object.values(item.post.acf).some((k) =>
                            component.activeFilters.includes(k)
                        )
                    );
                } else {
        ...
        async created() {
            ...
            if (component.items_type == 'objects') {
                component.$route.query.has_image != 'false'
                    ? (component.activeFilters = ['Has Image'])
                    : (component.activeFilters = []);
                // component.$route.query.has_image == 'true' ? component.activeFilters = ['Has Image'] : component.activeFilters = [];

                this.loadFilters();

                component.$route.query.maker
                    ? (component.aggregationMakerList = component.checkQueryArray(
                          component.$route.query.maker
                      ))
                    : (component.aggregationMakerList = []);
                component.$route.query.year
                    ? (component.aggregationYearList = component.checkQueryArray(
                          component.$route.query.year
                      ))
                    : (component.aggregationYearList = []);
                component.$route.query.type
                    ? (component.aggregationTypeList = component.checkQueryArray(
                          component.$route.query.type
                      ))
                    : (component.aggregationTypeList = []);
                ...
                if (
                    component.$route.query.sortby == 'accession' ||
                    component.$route.query.sort == null
                ) {
                ...
                this.page
                    ? this.getObjects(this.page, this.$route.query.search)
                    : this.getObjects(1);
                ...
                if (component.$route.query.chronology) {
                    ...
                    component.filters[1].items.find(
                        (item) => item.name == component.$route.query.chronology
                    ).active = true;
                } else {
                    // component.showPast = true;
                }
                ...
                if (component.$route.query.location) {
                    component.$route.query.location
                        ? (component.location = component.checkQueryArray(
                              component.$route.query.location
                          ))
                        : (component.location = []);
                ...
                this.page
                    ? this.getPosts(this.page, component.$route.query.search)
                    : this.getPosts(1);
                ...
                for (let itemNum = 0; itemNum < this.items; itemNum++) {
                    ...
                    if (
                        component.blockData[`items_${itemNum}_image`] ||
                        component.blockData[`items_${itemNum}_image`] === 0
                    ) {
                ...
                const postsSelectionData = await Promise.all(
                    postSelections.map(async (ps) => await this.getPost(...ps))
                );
                ...
                const mappedItems = [...Array(this.items)].map((item, i) => {
                    switch (component.blockData[`items_${i}_entry_type`]) {
                        case 'selection':
                            return {
                                post: postsSelectionData.find(
                                    (p) =>
                                        p.id ===
                                        `items_${i}_${
                                            component.blockData[`items_${i}_selection_type`]
                                        }_selection`
                                ),
                                heading: undefined,
                        ...
                        default:
                            return {
                                ...
                                paragraph_entry_type:
                                    component.blockData[`items_${i}_paragraph_entry_type`],
                                button: component.blockData[`items_${i}_button`],
                                image: imagesData.find(
                                    (img) => img.id === component.blockData[`items_${i}_image`]
                                ),
                                openNewTab: component.openNewTab(component.blockData, i),
                            };
                    }
        ...
        methods: {
            ...
            async getObjects(page, searchTerm) {
                ...
                if (this.page == undefined && this.$refs.filter) {
                    setTimeout(() => {
                        filterYPosition =
                            window.scrollY + component.$refs.filter.getBoundingClientRect().top;
                ...
                if (component.aggregationMakerList.length > 0) {
                    filterTerms.push({
                        terms: { 'Artist_Maker.sort_name': component.aggregationMakerList },
                    });
                }
                ...
                if (component.aggregationYearList.length > 0) {
                    filterTerms.push({
                        terms: { 'dates.era_labels': component.aggregationYearList },
                    });
                }
                ...
                const endpoint =
                    'https://ccma-search-proof-8365887253.us-east-1.bonsaisearch.net/stage';
                ...
                const searchQuery = {
                    ...
                    query: {
                        bool: {
                            should:
                                component.aggregationMakerList.length > 0 ||
                                component.aggregationMediumList.length > 0 ||
                                component.aggregationSupportList.length > 0 ||
                                component.aggregationYearList.length > 0 ||
                                component.aggregationTypeList.length > 0
                                    ? filterTerms
                                    : undefined,
                ...
                await axios
                    ...
                    .then((output) => {
                        ...
                        component.newItems = output.data.hits.hits.map((i) => {
                            const img =
                                i._source.Images.length > 0 ? i._source.Images[0] : undefined;
                            const imgUrl = img
            ...
            async getPosts(page, searchTerm) {
                ...
                await axios
                    .get(
                        `${component.interface.endpoint}categories?parent=${component.items_category}`
                    )
                    .then(async (output) => {
                        let categories;
                        let chr;
                        ...
                        if (
                            (component.showChronology == 'current' || component.showCurrent) &&
                            (component.items_type == 'events' ||
                                component.items_type == 'exhibitions')
                        ) {
                            chr = '&chronologies=9'; // SHOW ONLY CURRENT
                        } else if (
                            (component.showChronology == 'past' || component.showPast) &&
                            (component.items_type == 'events' ||
                                component.items_type == 'exhibitions')
                        ) {
                            chr = '&chronologies=8'; // SHOW ONLY PAST
                        } else if (
                            (component.showChronology == 'future' || component.showFuture) &&
                            (component.items_type == 'events' ||
                                component.items_type == 'exhibitions')
                        ) {
                        ...
                        console.log(
                            `${component.interface.endpoint}${
                                component.items_type == 'posts' ? 'posts' : component.items_type
                            }?categories_exclude=1${chr}${type}${meta_date}&categories=${
                                component.items_category
                            }${categories.length > 0 ? `,${categories.toString()}` : ''}&per_page=${
                                component.per_page
                            }&page=${page}${searchTerm ? `&search=${searchTerm}` : ''}${
                                component.alphabeticalOrder ? '&orderby=title' : ''
                            }${component.reverseOrder ? '&order=asc' : ''}`
                        );
                        await axios
                            .get(
                                `${component.interface.endpoint}${
                                    component.items_type == 'posts' ? 'posts' : component.items_type
                                }?_embed=wp:featuredmedia&categories_exclude=1${chr}${type}${meta_date}&categories=${
                                    component.items_category
                                }${
                                    categories.length > 0 ? `,${categories.toString()}` : ''
                                }&per_page=${component.per_page}&page=${page}${
                            ...
                            .then((output) => {
                                ...
                                if (component.alphabeticalOrder == false) {
                                    if (
                                        ...
                                        if (component.showChronology == 'future') {
                                            ...
                                            component.newItems.sort((a, b) =>
                                                component.items_type == 'events'
                                                    ? b.event_date.getTime() -
                                                      a.event_date.getTime()
                                ...
                                component.pagination = component.preparedPagination(
            ...
            imageUrlWidth(guid, width) {
                ...
                return `https://imagedelivery.net/O3WFf73JpL0l5z5Q_yyhTw/${guid
                    .replace('https://', '')
                    .replace('http://', '')
                    .replace('wp-content/uploads/', '')
                    .replace('wp-json/wp/v2/', '')}/w=${width},quality=75,format=webp`;
            },
            mapImageData(restData) {
                ...
                const desktop = {
                    ...
                    source_url: this.imageUrlWidth(restData.guid.rendered, 1800),
                };
                const mobile = {
                    aspect_ratio: imageAspect,
                    width: mobileWidth,
                    height: mobileWidth * imageAspect,
                    source_url: this.imageUrlWidth(restData.guid.rendered, 1200),
                };
                ...
                return {
                    ...
                    caption: {
                        rendered: restData.description.rendered
                            .replace(/<img[^>]*>/g, '')
                            .replace(/<p[^>]*>|<\/p>/g, '')
                            .replace(/\r?\n|\r/g, '')
                            .replace(/<a[^>]*>|<\/a>/g, ''),
            ...
            async getImage(i) {
                ...
                if (i) {
                    await axios.get(`${component.interface.endpoint}media/${i}`).then((output) => {
            ...
            loadFilters() {
                ...
                this.filters = filtersType.map((f) => ({
                    ...
                    items: f.items.map((i) => ({
                        name: i,
                        active:
                            i == 'Has Image' && component.activeFilters.includes('Has Image')
                                ? true
                                : false,
                    })),
                }));
