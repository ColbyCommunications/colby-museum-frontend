...
const getImage = async (img, endpoint) => {
    ...
    const desktop = {
        ...
        source_url: `https://imagedelivery.net/O3WFf73JpL0l5z5Q_yyhTw/${imageData.guid.rendered.replace('https://', '').replace('http://', '').replace('wp-content/uploads/', '').replace('wp-json/wp/v2/', '')}/w=1800,quality=75,format=webp`,
    };
    const mobile = {
        aspect_ratio: imageAspect,
        width: mobileWidth,
        height: mobileWidth * imageAspect,
        source_url: `https://imagedelivery.net/O3WFf73JpL0l5z5Q_yyhTw/${imageData.guid.rendered.replace('https://', '').replace('http://', '').replace('wp-content/uploads/', '').replace('wp-json/wp/v2/', '')}/w=1200,quality=75,format=webp`,
    };
    const mapped = {
        ...
        caption: {
            rendered: (imageData.description?.rendered || '')
                .replace(
                    /<div[^>]*class="[^"]*wp-video[^"]*"[^>]*>[\s\S]*?<\/video>\s*<\/div>/gi,
                    '',
                )
                .replace(/<(video|audio)[^>]*>[\s\S]*?<\/\1>/gi, '')
                .replace(/<img[^>]*>/g, '')
                .replace(/<p[^>]*>|<\/p>/g, '')
                .replace(/\r?\n|\r/g, '')
                .replace(/<a[^>]*>|<\/a>/g, ''),
