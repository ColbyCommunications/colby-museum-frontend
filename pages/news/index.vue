...
<script>
    ...
    export default {
        setup(props) {
            ...
            const endpointUrl = `${props.interface.endpoint}pages?slug=news`;
            useFetch(endpointUrl, {}).then(({ data, error, status }) => {
                if (error.value) {
            ...
            useSeoMeta({
                ogTitle: () =>
                    `${
                        pageMeta.value.title ? pageMeta.value.title?.rendered + ' | ' : ''
        ...
        async mounted() {
            ...
            await axios.get(`${this.interface.endpoint}pages?slug=news`).then((output) => {
